...
# =================== Extract Balance Components and Bank Details ===================
def extract_bank_and_ap(balance_data):
    """Extract bank balance, AP, and bank account details from balance sheet"""
    bank_total = 0.0
    ap_total = 0.0
    bank_details = []
    ...
    return round(bank_total, 0), round(ap_total, 0), bank_details

# =================== Extract Balance Components (for new formula) ===================
def extract_balance_components(balance_data):
    """
    Extracts Bank, Accounts Receivable, Prepaid Expenses, and Accounts Payable
    from Zoho Balance Sheet JSON with fine control:
      - AP: only first 'Accounts Payable' entry
      - Prepaid: only 'Prepaid Expenses' exact match
    """
    bank_total = 0.0
    ar_total = 0.0
    prepaid_total = 0.0
    ap_total = None  # None helps detect first occurrence
    ...
    def traverse_accounts(account_list, section_type):
        ...
        for acc in account_list:
            ...
            if section_type == "Assets":
                ...
                # 📦 Prepaid Expenses (exact match only)
                elif name == "Prepaid Expenses":
                    prepaid_total = total  # overwrite if found exact match

            # ------------- LIABILITIES SECTION -------------
            elif section_type == "Liabilities & Equities":
                # 📉 Accounts Payable (only first one)
                if "accounts payable" in lname and ap_total is None:
                    ap_total = total  # take only first match

            # Recurse deeper if nested accounts exist
    ...
    # Handle if AP never found (set to 0)
    if ap_total is None:
        ap_total = 0.0

    return round(bank_total, 0), round(ar_total, 0), round(prepaid_total, 0), round(ap_total, 0)
...
# =================== Process P&L Data (with expense details) ===================
...
def main():
    ...
    try:
        ...
        # =================== CASH AVAILABILITY SUMMARY ===================
        st.markdown("---")
        st.subheader("💰 Cash Availability Summary (Revised Formula)")

        # Fetch Balance Sheet Data for components
        bank_total, ar_total, prepaid_total, ap_total = extract_balance_components(balance_data)

        # Cash-Based Profit (from P&L)
        cash_based_profit = cash_metrics["Net Profit"]

        # Define Reserve (adjustable)
        min_reserve = st.number_input("Minimum Reserve (AED)", value=10000, step=1000)

        # Apply New Formula
        cash_available_for_withdrawal = (
            cash_based_profit + ar_total + prepaid_total - ap_total - min_reserve
        )
        ...
        with col1:
            st.metric("💵 Cash-Based P/L", f"AED {cash_based_profit:,.0f}")
        with col2:
            st.metric("📄 Accounts Receivable (AR)", f"AED {ar_total:,.0f}")
        with col3:
            st.metric("📦 Prepaid Expenses", f"AED {prepaid_total:,.0f}")
        with col4:
            st.metric("📉 Accounts Payable (AP)", f"AED {ap_total:,.0f}")
        with col5:
            st.metric(
                "💰 Cash Available for Withdrawal", 
                f"AED {cash_available_for_withdrawal:,.0f}",
                help="Cash-Based P/L + AR + Prepaid − AP − Reserve"
            )

        # Status message
        if cash_available_for_withdrawal <= 0:
            st.error("⚠️ No cash available for withdrawal — liabilities exceed assets.")
        elif cash_available_for_withdrawal < 50000:
            st.warning("⚠️ Limited cash available — consider reducing payables or increasing inflows.")
        else:
            st.success("✅ Healthy cash position — funds available for withdrawal.")
        ...
        with col2:
            ...
            st.write("**Withdrawal Calculation (Alternative Method):**")
            ...
            st.info(f"**Alternative Calculation: AED {withdrawal_info['available_profit']:,.2f}**")
        ...
        with tab6:
            ...
            **Cash-Based Performance Summary** ({from_date} to {to_date})
            ...
            • **Available for Withdrawal (New Formula):** AED {cash_available_for_withdrawal:,.2f}
            ...
            **Key Insights:**
            ...
            with col2:
                ...
                if bank_details:
                    ...
                    bank_df['Available for Withdrawal'] = cash_available_for_withdrawal
