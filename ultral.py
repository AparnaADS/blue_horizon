    ...
    def traverse(nodes: List[Dict], section: str):
        for node in nodes:
            ...
            if section == "assets":
                ...
                if "accounts payable" in name or "creditors" in name:
                    totals["ap"] += abs(total)
...
def main():
    ...
    dso = (ar_overview["total"] / revenue * period_days) if revenue else 0.0
    dpo = (ap_overview["total"] / expense_period_total * period_days) if expense_period_total else 0.0
    ...
    with tab_exec:
        ...
        row3 = st.columns(4)
        row3[0].metric("AR Outstanding", f"AED {ar_overview['total']:,.0f}", f"Overdue AED {ar_overview['overdue']:,.0f}")
        row3[1].metric("AP Outstanding", f"AED {ap_overview['total']:,.0f}", f"Overdue AED {ap_overview['overdue']:,.0f}")
        row3[2].metric(
            "Cash Runway",
    ...
    with tab_collections:
        ...
        col_col1.metric("AR Outstanding", f"AED {ar_overview['total']:,.0f}")
    ...
    with tab_operations:
        ...
        ops_cols[0].metric("AP Outstanding", f"AED {ap_overview['total']:,.0f}")
