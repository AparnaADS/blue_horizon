...
def extract_bank_and_ap(balance_data):
    bank_total = 0.0
    ap_total = None  # None helps us capture only first occurrence
    bank_details = []
                ...
                if liab.get("name") == "Liabilities":
                    for sub in liab.get("account_transactions", []):
                        if sub.get("name") == "Current Liabilities":
                            for acc in sub.get("account_transactions", []):
                                name = acc.get("name", "")
                                if name.strip() == "Accounts Payable":  # exact match only
                                    ap_total = float(acc.get("total", 0))
                                    print("✅ Final AP Found:", ap_total)
                                    break  # stop after first match

    return bank_total, ap_total if ap_total is not None else 0.0, bank_details
    ...
    try:
        ...
        with col2:
            st.metric(
                ...
                help=f"As of {to_date}"
            )
        
        with col3:
            st.metric(
                label="💰 Available for Withdrawal (AP Value)", 
                value=f"AED {accounts_payable:,.2f}",
                help="Showing Accounts Payable total fetched from Balance Sheet"
            )
